...
Jump to: [00-start](?p=00-start), [01-basic](?p=01-basic), [02-adding-items](?p=02-adding-items), [03-completing-items](?p=03-completing-items), [04-test-ui](?p=04-test-ui), [06-network](?p=06-network), [09-app-access](?p=09-app-access), [10-ci](?p=10-ci), [11-component-tests](?p=11-component-tests), [end](?p=end)
...
➡️ Pick the [next section](https://github.com/bahmutov/cypress-workshop-cy-vs-pw-half-day#contents) or go to the [00-start](?p=00-start) chapter
